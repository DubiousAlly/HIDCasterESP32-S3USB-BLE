...
void EspUsbHost::_onReceive(usb_transfer_t *transfer) {
  ...
  // }
  //EspUsbHost *usbHost = (EspUsbHost *)buffer_str.c_str()->context;
  //!Jmod
  ...
  if (endpoint_data->bInterfaceClass == USB_CLASS_HID) {
    if (endpoint_data->bInterfaceSubClass == HID_SUBCLASS_BOOT) {
      if (endpoint_data->bInterfaceProtocol == HID_ITF_PROTOCOL_KEYBOARD) {
        ...
        report.buttons = transfer->data_buffer[0];    //!Jmod was 1
        report.x = (uint8_t)transfer->data_buffer[1]; //!Jmod was 2
        report.y = (uint8_t)transfer->data_buffer[2]; //!Jmod was 4
        report.wheel = (uint8_t)transfer->data_buffer[3];  //!Jmod was 6
